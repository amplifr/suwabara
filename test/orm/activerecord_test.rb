...
class ActiveRecordTest < Minitest::Test
  def setup
    ...
    @img = Suwabara::StoredFile.new(@model, 'file', @tempfile, 'img.png')
  end
  ...
  def test_save_file_to_model
    ...
    assert_equal @model.image.name, 'img.png'
  end

  def test_dont_save_file_for_not_persisted_model
    @model1 = TestableModel.new
    @img = Suwabara::StoredFile.new(@model1, 'file', @tempfile, 'img.png')
    @model1.image = @img
  ...
  def test_save_file_for_persisted_model
    ...
    assert_equal "000/000/00#{@model1.id}/img.png", @model1.image.storage
